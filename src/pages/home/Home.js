...
function Home() {
  ...
  useEffect(() => {
    ...
    data.forEach((item) => {
      let taskDueDate = new Date(item.dueDate).toDateString();
  
      // Only show the alert if it's due today and hasn't been alerted yet
      if (taskDueDate === today && !alertedTasks.includes(item.title)) {
        // setTimeout(() => {
          new Promise((resolve,reject)=>{
            setTimeout(() => {
              window.alert(`Reminder: Task "${item.title}" is due today. Please complete it soon.`)
              resolve()
            }, 2000);
          })
          // window.alert(`Reminder: Task "${item.title}" is due today. Please complete it soon.`);
          setAlertedTasks((prevTasks) => [...prevTasks, item.title]); 
        // }, 2000);
      }
    });
  }, [tasksData, alertedTasks]);
